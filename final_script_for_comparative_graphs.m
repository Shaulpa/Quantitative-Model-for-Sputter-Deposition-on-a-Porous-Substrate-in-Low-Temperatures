...
c.NumWorkers = 8;
saveProfile(c);
parpool(8) %or a different number, depending on how many cores I have
num_of_points_in_subs_vec = 1000; %number of points in each 
% direction in the simulation
max_hight_points = 1200;
num_of_iterations = [120]; %array of ints
...
base_growth_steps_per_it = 4; %number of growth steps per iteration for non porous sample. 
...
num_of_matrix_pores = [36]; % array of ints
matrix_pore_rad_avg = [90]; %in pixels
matrix_pore_rad_std = [18]; %in pixels
...
por_at_height_std = ...
    ...
    length(num_of_matrix_pores));
%{
save('simulation_output.mat', ...
    'por_at_height_ave', 'por_at_height_std', ...
    'ave_height_ave', 'ave_height_std', ...
    'roughness_ave', 'roughness_std'); %check
%}
parfor sample_kind_it = 1:length(num_of_matrix_pores)
...
save('simulation_output.mat', ...
    'subs_porousity_percent_ave', 'subs_porousity_percent_std', ...
    'sputtered_porousity_percent_ave', 'sputtered_porousity_percent_std', ...
    'por_at_height_ave', 'por_at_height_std', ...
    'ave_height_ave', 'ave_height_std', ...
    'roughness_ave', 'roughness_std');
